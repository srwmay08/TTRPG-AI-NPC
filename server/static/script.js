...
function savingThrowBonus(abilityScore, proficientInSave = false, proficiencyBonus = 0) { const mod = getAbilityModifier(abilityScore || 10); return mod + (proficientInSave ? proficiencyBonus : 0); }
function calculateSkillBonus(baseStatScore, skillProficiencyValue, proficiencyBonus) { const modifier = getAbilityModifier(baseStatScore || 10); let skillBonus = modifier; if (skillProficiencyValue === 1) { skillBonus += proficiencyBonus; } else if (skillProficiencyValue === 2) { skillBonus += (proficiencyBonus * 2); } else if (skillProficiencyValue === 0.5) { skillBonus += Math.floor(proficiencyBonus / 2); } return skillBonus; }
function calculatePassiveSkill(baseStatScore, skillProficiencyValue, proficiencyBonus) { const skillBonus = calculateSkillBonus(baseStatScore || 10, skillProficiencyValue, proficiencyBonus); return 10 + skillBonus; }
function getProficiencyBonus(level) { level = level || 1; if (level < 1) return 2; if (level <= 4) return 2; if (level <= 8) return 3; if (level <= 12) return 4; if (level <= 16) return 5; return 6; }
...
async function triggerNpcGreeting(npcIdStr, npcName, dialogueRequestPayload) {
    ...
    try {
        ...
        renderAiSuggestions( // Ensure this function is correctly defined and called
...
async function toggleNpcInScene(npcIdStr, npcName) {
    ...
    if (isAdding && toggledNpc) {
        ...
        if (activePcIds.size > 0 || currentSpeakingPcId === "") { // Trigger greeting if PCs or DM event possible
...
async function associateHistoryFile() {
    ...
    if (!charIdToUse || charIdToUse === "null") { // Check for null string as well
...
async function dissociateHistoryFile(filename) {
    const charIdToUse = String(currentProfileCharId); 
    if (!charIdToUse || charIdToUse === "null") { alert("No character selected."); return; }
...
async function selectCharacterForDetails(charIdStr) {
    ...
    if (!charIdStr || charIdStr === "null") {
        console.warn("selectCharacterForDetails called with invalid charIdStr:", charIdStr);
        // Reset profile view if ID is invalid
        updateText('details-char-name', 'None');
        updateText('profile-char-type', '');
        updateText('profile-description', '');
        updateText('profile-personality', '');
        getElem('gm-notes').value = '';
        disableBtn('save-gm-notes-btn', true);
        getElem('npc-memories-collapsible-section').style.display = 'none';
        getElem('npc-faction-standings-section').style.display = 'none';
        getElem('character-history-collapsible-section').style.display = 'block'; // Keep history section potentially visible
        getElem('associated-history-list').innerHTML = '<li><em>Select a character.</em></li>';
        getElem('history-content-display').textContent = 'Select a character to view history.';
        currentProfileCharId = null; // Explicitly set to null
        return;
    }
    ...
    try {
        ...
        } else { // If PC is selected for profile view
...
function renderDetailedPcSheet(pcId) {
    ...
    // (This is the full logic for detailed PC sheet sections as provided in earlier scripts)
    const collapsibleSectionsData = [
        ...
        },
        {
            ...
            contentFn: () => {
                ...
                if (spellsByLevel['Pact Magic']) spellLevelsOrder.push('Pact Magic'); // Example for Warlocks, etc.
...
// --- Faction Standing UI ---
// (renderNpcFactionStandings and handleFactionStandingChange are defined above)

// --- Memory Management, Dialogue, Character Creation/GM Notes ---
// (renderMemories, addMemoryToCharacter, deleteMemory are defined above)
// (generateDialogue and renderAiSuggestions are defined above)
// (saveGMNotes and createCharacter are defined above)
    ...
    document.querySelectorAll('#left-column .collapsible-section').forEach(section => {
        ...
        if (header) {
            ...
            header.addEventListener('click', (e) => {
                if (e.target.closest('input[type="range"], select, button, textarea, input[type="text"]')) {
                    return; 
                }
                const content = section.querySelector('.collapsible-content');
                const contentToToggle = section.id === 'npc-faction-standings-section' ? getElem('npc-faction-standings-content') : content;

                if (contentToToggle) { 
                    section.classList.toggle('collapsed');
                    if(arrow) arrow.textContent = section.classList.contains('collapsed') ? ' ►' : ' ▼';
                    contentToToggle.style.display = section.classList.contains('collapsed') ? 'none' : 'block';
                }
            });
            
            const contentToToggle = section.id === 'npc-faction-standings-section' ? getElem('npc-faction-standings-content') : section.querySelector('.collapsible-content');
            const shouldBeOpen = ['pc-list-section', 'npc-list-section', 
                                  'character-profile-main-section', 'gm-notes-collapsible-section', 
            ...
            if (section.id === 'character-profile-main-section' && !section.classList.contains('collapsed')) {
                section.querySelectorAll(':scope > .collapsible-content > .collapsible-section > h3').forEach(subHeader => {
                ...
                const factionHeaderH4 = getElem('npc-faction-standings-section')?.querySelector('h4');
                if(factionHeaderH4) {
                    const factionSectionDiv = getElem('npc-faction-standings-section');
                    const factionArrow = factionHeaderH4.querySelector('.arrow-indicator');
                    const factionContentDiv = getElem('npc-faction-standings-content');
                    if (factionSectionDiv && !factionSectionDiv.classList.contains('collapsed')) {
                        if(factionArrow) factionArrow.textContent = ' ▼';
                        if(factionContentDiv) factionContentDiv.style.display = 'block';
                    } else if (factionSectionDiv) { 
                        if(factionArrow) factionArrow.textContent = ' ►';
                        if(factionContentDiv) factionContentDiv.style.display = 'none';
                    }
                }
            }
        }
